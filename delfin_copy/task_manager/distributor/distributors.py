...
from delfin.task_manager.distributor.telemetry.failed_telemetry_job \
    import FailedTelemetryJob
from delfin.task_manager.distributor.telemetry.telemetry_job import \
    TelemetryJob

LOG = log.getLogger(__name__)

distributor_BOOT_JOBS = [
...
class DistributorManager(object):
    def __init__(self, scheduler=None):
        if not scheduler:
            scheduler = BackgroundScheduler()
        self.distributor = scheduler
        self.distributor_started = False
    ...
    def start(self):
        """ Initialise the distributors for periodic job creation
        """
        if not self.distributor_started:
            self.distributor.start()
            self.distributor_started = True

        if not self.boot_jobs_scheduled:
            try:
                for job in distributor_BOOT_JOBS:
                    job_class = importutils.import_class(job)
                    job_instance = job_class(self.ctx)

                    # Create a jobs for periodic scheduling
                    job_id = uuidutils.generate_uuid()
                    self.distributor.add_job(job_instance, 'interval',
                ...
                # TODO: Currently failure of distributor is failing task manager
    ...
    def stop(self):
        ...
        for job_id, job in self.boot_jobs.items():
            self.distributor.remove_job(job_id)
            job.stop()
