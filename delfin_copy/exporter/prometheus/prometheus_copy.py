...
prometheus_opts = [
    ...
    cfg.IntOpt('timezone_offset',
               default=-1,
               help='default time zone offset (in sec) from UTC if '
                    'prometheus server is running on a different timezone'),
]
...
class PrometheusExporter(object):
    ...
    def _write_to_prometheus_format(self, f, metric, labels, values):
        ...
        for timestamp, value in values.items():
            prom_timestamp = self.convert_utc_to_prom_timez(timestamp)
            f.write("%s%s %f %d\n" % (metric, labels,
    ...
    def convert_utc_to_prom_timez(self, utc):

        to_zone = tz.tzlocal()
        # Check if time zone offset is set by user
        if cfg.CONF.PROMETHEUS_EXPORTER.timezone_offset != -1:
            to_zone = tz.tzoffset(cfg.CONF.PROMETHEUS_EXPORTER.timezone_offset)

        datetime_utc = datetime.date.fromtimestamp(utc / 1000)
        # Convert time zone
        prom_timez = datetime_utc.astimezone(to_zone)
        prom_timestamp = int(prom_timez.timestamp() * 1000)
        return prom_timestamp
